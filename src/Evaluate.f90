...
!   real p1(nlev+1), pp(nlev)
   ...
   character(len=50) sitename(300)
   real conc(300,3,200000), time(300,200000), yo, yopt, yorg
   integer no(nobs_max), date(300,200000)
   real lats(300,200000), lons(300,200000)
   real ylats, ylons
   real p0
   integer ic, ec
    ...
    do is=1, nsite
       ...
       do iis=1, no(is)
         if(abs(conc(is,1,iis)-conc(is,2,iis))<10.and.abs(conc(is,1,iis)-conc(is,3,iis))<10)then
            write(1,'(2f10.2,i10,f10.2,6f10.2)') lats(is,iis), lons(is,iis), date(is,iis), time(is,iis), &
                   conc(is,1,iis), conc(is,2,iis), conc(is,3,iis), -999, -999,-999
         else
             write(1,'(2f10.2,i10,f10.2,6f10.2)') lats(is,iis), lons(is,iis), date(is,iis), time(is,iis), &
                   -999, -999, -999, conc(is,1,iis), conc(is,2,iis), conc(is,3,iis)
         endif
       enddo
       close(1)
