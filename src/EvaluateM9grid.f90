   ...
   real, allocatable, dimension(:,:,:,:) :: Zm, rxco2
   real, allocatable, dimension(:,:,:) :: psm, rxco2s
   REAL yobs, yobs_xco2
   real HX(9), Hx_xco2(2)
   real ybias(9), xbias(2)
   integer nsrf, nxco2
   real Ax(nlevg,2)
   integer NumOfObs
   integer iday, sdate
   ...
   real lat_tmp,sec_tmp,lon_tmp,hgt_tmp, obs_tmp, unc_tmp, Hx_tmp(9), Robs_tmp
   integer i, j, k, ix1, ix2, jx1, jx2
...
!   real p1(nlev+1), pp(nlev)
   ...
   character(len=50) sitename(500)
   real conc(500,6,90000), time(500,100000), yo, yopt, yorg
!   real conc1(500,5,100000)
   integer no(nobs_max)
   integer date(500,100000)
   real lats(500,100000), lons(500,100000)
   real hgts(500,100000)
   real ylats, ylons
   real p0
   integer ic, ec
...
!      fileobs=trim(obsdir_srf)//'/Evalu_All/srf_'//sday
      fileobs='/data/home/TommyLv/GCASv3.2_fire/input/obs/EvaluObs/srf_'//sday
      ...
      if(alive)then
        ...
        !    print*, site_tmp, date_tmp, sec_tmp, lat_tmp, lon_tmp
        !     if(trim(site_tmp)=='wbi_tower-insitu_1_allvali')then
        !     print*, site_tmp, is, no(is)
        !     endif
!jiangf 2019.11.1
        ...
        !       ec=ec+ic-1
        !    endif
        !    class1=site_tmp(ic:ec)  !tower or surface or aircraft or shipboard
        ...
        !         ec=ec+ic-2
        !    endif
            !if(trim(site_tmp)=='calnex2010_aircraft-insitu')then
            !   print*, ic, ec, site_tmp(ic:ec)
            !endif 
        !    class2=site_tmp(ic:ec)  !flask or instu
           ! if(trim(class1)/='tower'.and.trim(class1)/='surface'.and.    &
           !        trim(class1)/='aircraft'.and.trim(class1)/='shipboard')then
           !     print*,'class1:', class1, site_tmp
           ! endif
           ! if(trim(class2)/='flask'.and.trim(class2)/='insitu'.and.trim(class2)/='insit'.and.trim(class2)/='pfp')then
           !     print*, 'class2:', class2, site_tmp
           ! endif
           ! print*, site_tmp(ic:ec)
        !    if(class2(1:4)=='insi') then
        !       if(mod(int(sec_tmp),3600)/=0) cycle
        !    endif
            if(lon_tmp<0)then
               lon_tmp=lon_tmp+360
            endif
          ...
          ! if(trim(site_tmp)=='rgl_tower-insitu_160_allva')then
          !  print*, site_tmp,secs(tx), sec_tmp, abs(sec_tmp-secs(tx))
          ! endif
            kx_tmp=dmin2(hgt_tmp,Zm(ix_tmp,jx_tmp,:,tx),nlev)
        !    if(sample_flag==1)then
        !       if(tx==1)then
        !          Hx_tmp(1:2)=(co2(ix_tmp,jx_tmp,kx_tmp,tx,1:2)+co2(ix_tmp,jx_tmp,kx_tmp,tx+1,1:2))/2.0
        !       elseif(tx==8)then
        !          Hx_tmp(1:2)=(co2(ix_tmp,jx_tmp,kx_tmp,tx,1:2)+co2(ix_tmp,jx_tmp,kx_tmp,tx-1,1:2))/2.0
        !       else
        !          Hx_tmp(1:2)=(co2(ix_tmp,jx_tmp,kx_tmp,tx,1:2)+co2(ix_tmp,jx_tmp,kx_tmp,tx+1,1:2) &
        !                              +co2(ix_tmp,jx_tmp,kx_tmp,tx-1,1:2))/3.0
        !       endif
        !    else
               ix1=ix_tmp-1
               ix2=ix_tmp+1
               ...
               jx2=jx_tmp+1
               if(jx1==0) jx1=2
               if(jx2==nlat+1) jx1=nlat-1
               Hx_tmp(1)=co2(ix_tmp,jx_tmp,kx_tmp,tx,1)
               Hx_tmp(2)=co2(ix2,jx_tmp,kx_tmp,tx,1)
               Hx_tmp(3)=co2(ix1,jx_tmp,kx_tmp,tx,1)
               Hx_tmp(4)=co2(ix_tmp,jx2,kx_tmp,tx,1)
               Hx_tmp(5)=co2(ix_tmp,jx1,kx_tmp,tx,1)
               Hx_tmp(6)=co2(ix1,jx1,kx_tmp,tx,1)
               Hx_tmp(7)=co2(ix1,jx2,kx_tmp,tx,1)
               Hx_tmp(8)=co2(ix2,jx2,kx_tmp,tx,1)
               Hx_tmp(9)=co2(ix2,jx1,kx_tmp,tx,1)
        !    endif
               Hx_tmp(1:9)=Hx_tmp(1:9)*29/44*1.0e6
       !     Hx_tmp(1:2)=co2(ix_tmp,jx_tmp,kx_tmp,tx,1:2)*29/44*1.0e6
       !      if(abs(Hx_tmp(1)-obs_tmp)>10) cycle
       !     if(trim(class1)=='aircraft')then
       !     write(110,'(a26,i10,f10.1,3f10.2,3f10.2)') site_tmp, date_tmp, sec_tmp, lat_tmp, lon_tmp,hgt_tmp, obs_tmp, Hx_tmp(1:2)
       !     else
       !     write(100,'(a26,i10,f10.1,3f10.2,3f10.2)') site_tmp, date_tmp, sec_tmp, lat_tmp, lon_tmp,hgt_tmp, obs_tmp, Hx_tmp(1:2)
       !     endif 
       !    if(site_tmp(ic:ec)/='aircraft')then
           if(is==0)then
                 is=is+1
                 ...
                 hgts(is,1)=hgt_tmp
                 conc(is,1,1)=obs_tmp
                 conc(is,2:6,1)=Hx_tmp(1:5)
              !   conc1(is,1:5,1)=Hx_tmp(5:9)
               !  conc(is,3,1)=Hx_tmp(2)
                 ...
                 !    endif
                        ...
                        conc(s,2:6,iis)=Hx_tmp(1:5)
              !          conc1(is,1:5,iis)=Hx_tmp(5:9)
               !         conc(s,2:10,iis)=Hx_tmp(1:9)
        ...
        !    endif
         ...
         !   if(abs(Hx_tmp(1)-obs_tmp)<10.and.abs(Hx_tmp(2)-obs_tmp)<10)then
               yobs=yobs+obs_tmp
               Hx(1:9)=Hx(1:9)+Hx_tmp(1:9)
               ybias(1:9)=ybias(1:9)+Hx_tmp(1:9)-obs_tmp
...
100 continue
      ...
      file_gosat=trim(obsdir_oco2)//'/oco2_v10_'//sday//'.nc'
   ...
   ! print*, ybias/nsrf
    ...
    do is=1, nsite
       ...
       do iis=1, no(is)
        ! if(abs(conc(is,1,iis)-conc(is,2,iis))<10.and.abs(conc(is,1,iis)-conc(is,3,iis))<10)then
            write(1,'(2f10.2,f10.1,i10,f10.2,6f10.2)') lats(is,iis), lons(is,iis),hgts(is,iis), date(is,iis), time(is,iis), &
                   conc(is,1:6,iis)  !, conc1(is,1:5,iis)
        ! else
        !     write(1,'(2f10.2,f10.1,i10,f10.2,6f10.2)') lats(is,iis), lons(is,iis),hgts(is,iis), date(is,iis), time(is,iis), &
        !           -999.0, -999.0, -999.0, conc(is,1,iis), conc(is,2,iis), conc(is,3,iis)
        ! endif
       enddo
       close(1)
   ...
   do k=1, nr
     ...
     open(1,file=trim(opt_dir)//'/'//trim(casename)//'/'//'xco2_'//trim(regname(k))//'.txt')
     do iy=syear, eyear
        if(syear==eyear)then
          sm=smon
          em=emon
